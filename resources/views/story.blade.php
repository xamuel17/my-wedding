...
@section('title', 'Our Story - Faith & Samuel — Together Forever')
            ...
            <div class="relative">
                ...
                <div class="space-y-8 sm:space-y-12">
                    @foreach($events as $i => $event)
                        <div class="relative flex items-start md:items-center {{ $i % 2 === 0 ? '' : 'md:flex-row-reverse' }}"
                             data-aos="{{ $i % 2 === 0 ? 'fade-right' : 'fade-left' }}" data-aos-delay="{{ $i * 100 }}">

                            <!-- Card (mobile: always right; desktop: alternates) -->
                            <div class="ml-16 md:ml-0 md:w-[calc(50%-2rem)] {{ $i % 2 === 0 ? 'md:pr-8 md:text-right' : 'md:pl-8' }}">
                                <div class="bg-white p-5 sm:p-6 rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-400 {{ $event['highlight'] ? 'border-2 border-royal-purple ring-2 ring-royal-purple/10' : 'border border-gray-100' }}">
                                    <p class="text-xs text-lilac font-nunito font-semibold uppercase tracking-wider mb-1">{{ $event['date'] }}</p>
                                    <h4 class="font-cormorant text-xl text-royal-purple mb-2">{{ $event['title'] }}</h4>
                                    <p class="text-gray-500 text-sm leading-relaxed">{{ $event['text'] }}</p>
                                </div>
                            </div>
