                    ...
                    @foreach($navItems as $item)
                        <li>
                            <a href="{{ route($item['route']) }}"
                               @click="sidebarOpen = false"
                               class="nav-link flex items-center gap-3 py-3 px-4 rounded-lg text-gray-600 hover:text-royal-purple hover:bg-royal-purple/5 transition-all duration-300 font-cormorant text-lg {{ request()->routeIs($item['route']) ? 'text-royal-purple bg-royal-purple/5 font-semibold' : '' }}">
                                <i class="{{ $item['icon'] }} text-sm {{ request()->routeIs($item['route']) ? 'text-royal-purple' : 'text-lilac' }}"></i>
                                {{ $item['label'] }}
                            </a>
                        </li>
                    @endforeach
